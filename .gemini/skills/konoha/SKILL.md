...
3. **Bridge Extension Sync & CLI Distribution (Antigravity IDE ONLY)**:
   - On fresh installation (`konoha init`) and upgrade (`konoha upgrade`), `https://github.com/andycungkrinx91/konoha-bridge` is cloned from live `master`, packaged into `konoha-bridge-1.6.0.vsix` via `@vscode/vsce package`, and installed ONLY into Antigravity IDE (`antigravity --install-extension`). The VSIX is NEVER installed into any other IDE (no `code --install-extension`, no `cursor --install-extension`). When Antigravity IDE is present, an atomic directory sync into `~/.antigravity-ide/extensions/andycungkrinx91.konoha-bridge-master-universal/` is also performed with `extensions.json` registration. Fallback VSIX is bundled in `assets/konoha-bridge-1.6.0.vsix`.
...
11. **Zero-AI-Slop Pre-Gate & `aislop` MCP Integration**:
    ...
    - Kage Review Pre-Gate & Minimum Confidence Gate: requires `ai_slop_clean: true` and `ai_slop_findings: 0` in `kage_review.json` before confidence scoring, and requires overall and per-category confidence Minimum Required: ≥ 98% to approve workflow delivery.
...
16. **Single Database Access Layer & Hybrid Multilingual Vector Search Invariants**:
    ...
    - `src/vector_search.js` provides semantic search using IBM Granite 97M Multilingual ONNX (`onnx-community/granite-embedding-97m-multilingual-r2-ONNX`, 384-dim, CLS pooling, L2 normalization) and `Xenova/ms-marco-MiniLM-L-6-v2` (MS MARCO MiniLM cross-encoder) reranking, running natively via `@huggingface/transformers`.
...
33. **Pure Node.js Single-Runtime & Protocol Stream Isolation Invariants (v2.0.0-beta.7)**:
    - 100% Pure Node.js Single-Runtime: Complete elimination of Python 3 runtime and dependencies. All 35 canonical MCP tools, 16 CLI commands, 7 ninja subagents, and 7 client integrations run natively on Node.js (v18–v26). Zero Python processes are spawned at runtime, and zero `.py` files remain in active execution paths.
    - Native ONNX Embeddings & Vector Search: IBM Granite 97M Multilingual ONNX neural embeddings (`onnx-community/granite-embedding-97m-multilingual-r2-ONNX` via `@huggingface/transformers`) and `Xenova/ms-marco-MiniLM-L-6-v2` (MS MARCO MiniLM cross-encoder) run directly in-process in `src/vector_search.js`, caching models under `~/.konoha/models/` and achieving ±0.0001 cosine similarity parity with PyTorch baselines.
...
42. **Elevated Kage Reviewer Minimum Confidence Threshold Invariant (≥ 98%) (v2.0.0-beta.7)**:
    - Strict Delivery Gate: In `src/mcp/workflow.js`, test suites, and documentation, the Kage Reviewer confidence gating threshold requires `Minimum Required: ≥ 98%` across all verification categories. Delivery is mechanically blocked if overall or category confidence < 98%.
43. **VSIX Antigravity-Only Scoping & Models Command Consolidation Invariants (v2.0.0-beta.7)**:
    - VSIX Install Scope: `autoInstallKonohaBridgeExtension` in `bin/cli.js` installs `konoha-bridge-1.6.0.vsix` ONLY into Antigravity IDE (`installExtensionViaCli('antigravity', ...)`). NEVER re-add `code` or `cursor` VSIX installation calls — the extension is exclusively an Antigravity IDE integration.
...
56. **Premium Flow GIF Generator Invariants (v2.0.0-beta.7)**:
    ...
    - Scene Fidelity: the orchestration scene must mirror the README Mermaid spec (steps 1-7, remediation loop `<= 7 cycles`, 83-98% token savings badge), the jonin scene must mirror the actual pipeline (build spec -> Taste-Skill dials 8/7/6 -> human-built fingerprint -> anti-slop gate -> Kage >= 98% -> delivered), and the kage scene must mirror the real gate (aislop scan 100/100 -> anti-slop rules 0 findings -> verification matrix -> >= 98% confidence with BLOCKED below threshold).
...
61. **Vibes-Plug Skills Integration & Unified Routing Invariant (v2.0.0-beta.7)**:
    - **119 Curated Skills**: Merged curated skills from `https://github.com/roedyrustam/vibes-plug` into canonical ninja agent skill references (`anbu-skill`, `jonin-skill`, `kage-skill`, `chunin-skill`, `genin-skill`, `tokubetsu-jonin-skill`, `sannin-skill`).
    - **Routing Aliases**: `src/mcp/skills.js` registers 190 canonical aliases in `ALIASES` (`normalizeLegacySkillName`), allowing agents and users to discover and retrieve skills by short name or domain prefix seamlessly.
    - **Cross-Client Synchronization**: All 119 skills and assets propagate across 5 mirror trees via `rtk node scripts/sync_skills.js` and index into SQLite FTS5 and IBM Granite vector chunks.
62. **Document AI Detector & ZeroGPT 0%–3% Precision Defense (v2.0.0-beta.7)**:
    - **Pure Core + Multi-Surface**: `src/docs_ai_detector.js` inspects `.docx`, `.pdf`, `.pptx`, `.xlsx`, `.md`, and `.txt` files across MCP tool (`docs_ai_detector`), CLI (`konoha detect-docs <path>`), and Web UI (`apps/web/src/components/Detector.svelte`).
    - **ZeroGPT Sliding-Window Burstiness**: Implements `analyzeParagraphBurstiness` (`ZEROGPT-01`) evaluating per-paragraph sentence cadence ($CV \ge 0.85$ target, $CV < 0.28$ warning) and AI transition trigger density to enforce our 0.0%–3.0% ZeroGPT risk target.
    - **Container & Metadata Sanitization**: Unpacks DOCX containers to extract true paragraph boundaries and inspects both `docProps/core.xml` and `docProps/app.xml` to detect unscrubbed library footprints (`python-docx`, `docx-js`, etc.), watermarks, and dark theme violations.
63. **v2.0.0-beta.7 16-Bug Comprehensive Hardening & Parity Invariant**:
    - **AI Slop Gate Circuit Breaker & Safe Degrade**: In `src/mcp/workflow.js`, `runAislopGate()` utilizes `CircuitBreaker` (threshold=2, recovery=120s) so that if the external aislop process hangs, times out, or fails repeatedly, the gate degrades to advisory mode instead of blocking developers. Cross-platform execution prefers `rtk` first with fallback to `aislop` via `which`/`where`, with `killSignal: 'SIGTERM'`.
    - **Dynamic File mtime Cache Invalidation**: `getMaxMtime()` dynamically computes the latest mtime across changed project files and invalidates cached `aislop_gate` results when file mtimes exceed `cached_at`, preventing stale score approvals.
    - **Mechanical 98% Confidence & 100/100 Aislop Gate Constants**: `MINIMUM_CONFIDENCE = 98` and `AISLOP_TARGET_SCORE = 100` are exported as constants and enforced mechanically without heuristic drifts.
    - **Universal 7-Client Workflow Reminder & Mandatory First Action**: `src/workflow_reminder.js` detects and emits reminders across all 7 clients (Antigravity, Cursor, OpenCode, Pi, Codex, Claude Code, CommandCode), mandating `FIRST ACTION: call konoha.find_skill with keywords BEFORE any code changes`.
    - **High-Effort Instruct Base Personality & 10 Forbidden Fillers**: Across `src/mcp/memory_reporting.js`, `src/agent_contract.js`, and `src/workflow_reminder.js`, 10 conversational filler phrases are strictly banned (`Hmmmm`, `Let me check`, `Let me see`, `Wait, let me`, `Wait - but`, `I will now proceed to`, `Let me examine`, `let me`, `hmm`, `hmmm`). Output format strictly follows ADHD-friendly structured execution with zero monologue leaks.
    - **Document AI Detector 0%–1% Precision Recalibration & Text Paste Mode**: `src/docs_ai_detector.js` filters out structural elements (markdown headers, all-caps titles, bullet points, numbered lists, labels) from burstiness calculation to eliminate false-positive AI flags. Recalibrates burstiness threshold to 0.18, paragraph CV to 0.25, cliché thresholds (8 high, 5 medium), and transition triggers (minimum 3 required). Adds Web UI text-paste mode (`apps/web/src/components/Detector.svelte`) backed by `POST /api/v1/detect-docs/text`.
    - **Skill Install Full Dual Indexing (FTS5 + Vector)**: Removed `--skip-embeddings` from `addSkillDirect()` and `createSkillFromTemplate()` in `src/skill_manager.js`, ensuring newly installed skills from skills.sh or Git repositories are immediately searchable via both FTS5 keyword matching and Granite neural vector search.
    - **Cross-Client 7-Tree Skill Parity**: `scripts/sync_skills.js` includes `.codex/skills` and `.opencode/skills` in `CLIENT_MIRRORS`, ensuring complete parity across all 7 clients when directories exist.
    - **SDLC Task Governance Integration**: Kage review gate directly checks `sdlc_manager.listTasks({ sessionId })` and blocks delivery if any task is explicitly in `blocked` or `failed` status.
    - **Official Skills Deduplication**: Standalone `helm-chart-scaffolding` is permanently removed as an independent skill root and retained exclusively as an internal reference under `anbu-skill/references/helm-chart-scaffolding.md`. Canonical aliases `agent-browser`, `devsecops-engineer`, `documentation`, `modern-full-stack`, and `websearch-deep` are registered in `src/mcp/skills.js`.
63. **Clean Runtime Reinstallation & ~/.konoha Sanitization Invariant (v2.0.0-beta.7)**:
    - **Clean Reinstall Execution**: Running `konoha init --force --yes` (or alias `reinstall`, `konoha clean`) invokes `cleanKonohaRuntimeDir()` in `src/deploy_utils.js`.
    - **Stale Disk Bloat Elimination**: Automatically purges stale backups (`konoha.db.bak-*`, `skills.db.bak-*`, `*.bak`), legacy databases (`skills.db*`), legacy root Python scripts (`*.py`, `*.pyc`), dead PID/lock files (`bridge.pid`, `ui.pid`, `package-lock.json`), dead caches (`transcript_cache.json`), transient scratch tasks in `tmp/`, misplaced root `.vsix` packages, obsolete root `cli.js`, and orphaned root `.js` files not in `src/`.
    - **Critical Asset Preservation**: Strictly preserves `konoha.db` (active SQLite DB) and its WAL/SHM, `transformers_cache/` (neural model weights), `node_modules/` (production dependencies), `vendor/`, `searxng/`, and `web_token`.
    - **Safe Database Maintenance**: Runs safe WAL truncation (`wal_checkpoint(TRUNCATE)`) and database vacuuming (`VACUUM`) on `konoha.db` to defragment storage and reclaim disk space.
    - **Safety Target Rejection**: Rejects dangerous targets (root `/`, user `HOME`, or matching `srcDir`) to prevent accidental deletion.
64. **AI Slop Scanner Zero-Exclusions & Mandatory Force-Scan Invariant (v2.0.0-beta.7)**:
    - **Never Exclude Anything Except Dependency Modules**: The AI slop scanner enforces an absolute zero-exclusions policy. ONLY third-party dependency modules and framework package libraries (`node_modules/`, `vendor/` from composer/go, `.venv/`, `venv/`, `*/site-packages/`, `Pods/`, `target/`, `.gradle/`, `.pub-cache/`, `.cargo/`, `.nuget/`) may ever be excluded.
    - **Zero Blanket Exclusions**: Application source code, tests, components, configs, templates, scripts, and documentation files must NEVER be excluded or ignored under any circumstance.
    - **Mandatory Force-Scan**: `runAislopGate()` removes any configuration file prerequisite (`.aislop/config.yml` is no longer required) and always force-scans the project before synthesis. Delivery is strictly blocked if the mechanical scan score < 100 or findings > 0.
65. **Collapsible Submenu Accordion & Expandable Content Canvas Invariant (v2.0.0-beta.7)**:
    - **Clickable Submenu Accordion**: Sidebar navigation sections (`Overview`, `Clients & Finance`, `Tools & Integrations`, `Configuration`, `Operations`, `Resources`) feature interactive collapsible accordions. Clicking any parent category expands or collapses its sub-menu items with smooth animated chevron transitions. The active section auto-expands on route changes.
    - **Sidebar Collapse for Maximum Center Space**: A dedicated sidebar toggle button in both the sidebar brand header and top main header enables closing and opening the sidebar. When collapsed (`w-0`), the central main content stage expands to 100% full screen width, giving users maximum focus area.
66. **Chidori Azure Default, 4-Gradient Color Palette & 5px Card Radius Invariant (v2.0.0-beta.7)**:
    - **Chidori Azure Default Theme**: The out-of-the-box default theme is `chidori` (Chidori Azure Light), delivering sky-blue and cyan lightning focus with pure light mode eye-friendly contrast.
    - **Universal 4-Gradient Theme Style**: All 10 themes in the Konoha palette define a 4-color gradient array (`--theme-grad-1` through `4`, `--theme-4gradient`, `--theme-4gradient-bar`, `--theme-4gradient-subtle`), displayed with visual 4-color swatches in the theme switcher modal and as a top accent bar.
    - **Distinctive Akatsuki Dusk Plum Palette**: `akatsuki` uses a rich Dusk Plum / Fuchsia palette (`#86198f`, `#c026d3`, 4-stop gradient `['#581c87', '#701a75', '#86198f', '#c026d3']`, `#fdf4ff`), completely eliminating duplicate hues with Sharingan Rose Light (`#dc2626`).
    - **Crisp 5px Card Border-Radius Invariant**: Every card container (`.glass-card-3d`, `.theme-card`, `.card`, modals, and metric tiles) enforces a strict 5px border-radius (`border-radius: 5px !important`).
    - **Strong Glassmorphism**: High-performance frosted glass panels feature 28px backdrop blur, 195% saturation, enhanced specular highlight edges (`--glass-edge`, `--glass-highlight`), and GPU-accelerated translateZ layering.

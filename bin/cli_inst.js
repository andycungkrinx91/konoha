...
function ensureAutoSetup() {
  ...
  const __mark = (n) => process.stderr.write('\n[TSEG] '+n+' '+(Number(process.hrtime.bigint()-__t0)/1e6).toFixed(1)+'ms\n');
  __mark('start');
  // 1. Ensure the directories exist
  const dirs = [
    path.join(HOME, '.gemini'),

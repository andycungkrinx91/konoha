...
async function callRawInference(ctx, messages, modelEnum, tools = null, images = []) {
  if (ctx.bridgeConfig && ctx.bridgeConfig.provider === 'openai') {
    return callOpenAiInference(ctx, messages, modelEnum, tools, images);
  }
...
async function callOpenAiInference(ctx, messages, modelEnum, tools = null, images = []) {
  ...
  if (!targetUrl) {
    throw new Error('OpenAI bridge targetUrl is not configured');
  }
  ...
  const model = ctx.requestedModel || 'gpt-4o';
  log(ctx, `📡 Forwarding OpenAI request to: ${requestUrl} (model: ${model})`);
  ...
  if (!response.ok) {
    ...
    err.isUpstreamError = true;
    err.rawError = parsedJson;
    throw err;
  }
  ...
  if (!choice || !choice.message) {
    throw new Error('Invalid response structure from OpenAI provider');
  }

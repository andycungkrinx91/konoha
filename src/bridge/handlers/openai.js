...
async function _handleChatCompletionsInner(
  ...
  try {
    ...
    let errType = isRateLimit ? 'rate_limit' : 'server_error';
    let message = `Upstream model provider error: ${extractProviderError(err.message)}`;
    ...
    log(
      ctx,
      `🛑 [${err.isUpstreamError ? 'upstream' : isRateLimit ? 'rate_limit' : 'server_error'}→${status}] returning ${status} (Retry-After: ${retryAfterSecs}s): ${err.message.substring(0, 120)}`,
    );
    ...
    if (err.isUpstreamError && err.rawError?.error?.code) {
      errPayload.error.code = err.rawError.error.code;
    }
    ...
    if (isStream && streamState.headersSent) {
      ...
      // Stream NOT yet committed — return a proper HTTP status code

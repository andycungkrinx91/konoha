...
function loadBridgesFromMcp() {
  ...
  const os = require("os");
  const path = require("path");
  const bridgesPath = path.join(os.homedir(), ".konoha", "bridges.json");
  if (!fs.existsSync(bridgesPath)) {
    ...
    try {
      ...
      fs.writeFileSync(bridgesPath, JSON.stringify(defaultBridges, null, 2) + "\n");
    } catch (e) {}
    return defaultBridges;
  }
  try {
    return JSON.parse(fs.readFileSync(bridgesPath, "utf8"));
  } catch (err) {
    return [];
  }
}
  ...
  try {
    ...
    for (const name of activeBridges.keys()) {
      if (!enabledNames.has(name)) {
        ...
        process.stderr.write(`[bridge:${name}] Stopping bridge server...\n`);
        try {
    ...
    for (const b of enabledBridges) {
      ...
      if (!active) {
        ...
        process.stderr.write(
          `[bridge:${b.name}] Starting server on port ${b.port} (${b.provider})...\n`,
        );
        try {
...
function main() {
  ...
  syncBridges()
    .then(() => {
      ...
      return startGateway(activeBridges, 11434);
    })
    .catch((err) => {
      process.stderr.write(`[bridge] Initial sync / gateway failed: ${err.message}\n`);
    });

  // Periodically check bridges.json for updates (every 5 seconds)
  ...
  // Watch bridges.json if possible for instant response
  const fs = require("fs");
  const os = require("os");
  const path = require("path");
  const bridgesPath = path.join(os.homedir(), ".konoha", "bridges.json");
  ...
  if (fs.existsSync(bridgesPath)) {
    try {
      watcher = fs.watch(bridgesPath, (eventType) => {
        if (eventType === "change") {
          syncBridges().catch(() => {});
        }
      });

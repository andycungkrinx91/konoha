...
test('mirrorSkillsDirectory returns 0 (no-op)', () => {
  const { mirrorSkillsDirectory } = require('../src/deploy_utils');
...
test('syncCursorSkillsFromAgents returns 0 (no-op)', () => {
  const { syncCursorSkillsFromAgents } = require('../src/deploy_utils');
...
test('getCursorStatus counts skills from ~/.agents/skills (canonical source)', () => {
  const { getCursorStatus } = require('../src/cursor_manager');
  ...
  if (fileExists(agentsSkills)) {
    ...
    // Just verify it's a reasonable number (not 0 and not huge)
    if (status.skillsGlobal < 5 || status.skillsGlobal > 50) {
      throw new Error(`Unexpected skillsGlobal count: ${status.skillsGlobal}`);
    }
  }
});
...
test('skill_manager.removeSkill only removes canonical path', () => {
  ...
  const src = fs_real.readFileSync(path.join(__dirname, '../src/skill_manager.js'), 'utf-8');

...
Validates the complete pipeline:
  prompt.md -> sannin (routing) -> delegate.md -> <agent> -> result.md

Covers four phases:
  Phase 1: Sannin keyword-routing heuristics (one test per agent + default).
  Phase 2: Full delegation cycle for anbu (5-step protocol).
...
class TestAgentDelegation(unittest.TestCase):
    """End-to-end test of sannin -> <agent> -> result.md pipeline."""
    ...
    def test_route_backend_prompts_to_anbu(self):
        ...
        self.assertEqual(res["status"], "routed")
        self.assertEqual(res["selected_agent"], "anbu")

    def test_route_architecture_prompts_to_kage(self):
        """Architecture/security keywords route to kage."""
        ...
        self.assertEqual(res["status"], "routed")
        self.assertEqual(res["selected_agent"], "kage")

    def test_route_research_prompts_to_chunin(self):
        """Research/documentation keywords route to chunin."""
        ...
        self.assertEqual(res["status"], "routed")
        self.assertEqual(res["selected_agent"], "chunin")

    def test_route_docs_prompts_to_tokubetsu_jonin(self):
        """Documentation keywords route to tokubetsu_jonin."""
        ...
        self.assertEqual(res["status"], "routed")
        self.assertEqual(res["selected_agent"], "tokubetsu_jonin")

    def test_route_exploration_prompts_to_genin(self):
        """Codebase exploration keywords route to genin."""
        ...
        self.assertEqual(res["status"], "routed")
        self.assertEqual(res["selected_agent"], "genin")

    def test_default_routes_to_kage_when_no_keywords(self):
        """Prompts with no agent keywords fall back to kage."""
    ...
    def test_full_delegation_cycle_with_anbu(self):
        ...
        res1 = json.loads(server.run_sannin(task_dir=task))
        self.assertEqual(res1["status"], "routed")
        self.assertEqual(res1["selected_agent"], "anbu")
        ...
        # Step 4: Call anbu -> it reads delegate.md, loads skill, returns instructions
        res2 = json.loads(server.run_mcp_agent(agent_name="anbu", task_dir=task))
        self.assertEqual(res2["status"], "ready")
        self.assertEqual(res2["phase"], "execution")
        self.assertEqual(res2["agent"], "anbu")
        ...
        # Step 6: Call sannin again — should return the completed result
        res3 = json.loads(server.run_sannin(task_dir=task))
    ...
    def test_anbu_loads_anbu_skill_into_context(self):
        """anbu's returned instructions embed the anbu-skill content."""
        task = self._setup_task(
            "anbu_skill",
            delegate_text="---\nyaml: foo\n---\n\nFix this API bug end-to-end.\n",
        )
        res = json.loads(server.run_mcp_agent(agent_name="anbu", task_dir=task))
    ...
    def test_jonin_loads_jonin_skill_with_references(self):
        """jonin loads jonin-skill plus any reference docs from the DB."""
        task = self._setup_task(
            "jonin_skill",
    ...
    def test_sannin_routing_includes_delegate_instructions(self):
        """sannin's instructions tell the caller to write delegate.md and call the agent."""
        task = self._setup_task("sannin_instr", prompt_text="build a sveltekit landing page component")
        res = json.loads(server.run_sannin(task_dir=task))
        self.assertEqual(res["status"], "routed")
        instructions = res["instructions"]
        # Selected agent surfaced with backticks (server.py:1848)
        self.assertIn("**Selected Agent**: `jonin`", instructions)
        ...
        self.assertIn("Call `jonin`", instructions)
...
# Set Claude Code env signal
        ...
        if "AGY_SESSION" in os.environ:
            del os.environ["AGY_SESSION"]

        task = self._setup_task("attribution", prompt_text="do the thing")
        server.run_sannin(task_dir=task)  # this triggers log_tool_call

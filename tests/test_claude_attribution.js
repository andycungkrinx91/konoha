...
function claudeTranscriptLine(text, taskSubagent = null) {
...
function setupClaudeSession(text, mtimeOffset = 7200) {
  const projectSlug = 'konoha-claude-attribution-test';
  const convId = `test-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
  const convDir = path.join(PROJECTS, projectSlug);
  fs.mkdirSync(convDir, { recursive: true });
  const transcriptPath = path.join(convDir, `${convId}.jsonl`);
  fs.writeFileSync(transcriptPath, claudeTranscriptLine(text) + '\n', 'utf8');
...
function main() {
  ...
  for (let idx = 0; idx < AGENTS.length; idx++) {
    ...
    const [convDir] = setupClaudeSession(text, 7200 + idx * 120);
    try {
  ...
  const [convDir] = setupClaudeSession(
  ...
  const [convDirTask, transcriptPath] = setupClaudeSession(
    '[Konoha] orchestrator active. Delegating to anbu.',
    7200 + (AGENTS.length + 1) * 120
  );
  fs.appendFileSync(transcriptPath, claudeTranscriptLine('Delegating.', 'anbu') + '\n', 'utf8');
  ...
  try {
    ...
    fs.rmSync(convDirTask, { recursive: true, force: true });
  }

...
## Project Scaffolding & Strict Configuration

When scaffolding or repairing a Svelte project from zero, you MUST strictly configure `package.json`, `svelte.config.js`, and `vite.config.ts` to match the exact structures below. DO NOT rely on generic `create-svelte` defaults which often result in plain Vite SPAs.
...
### 4. src/app.css (Design System Core)
...
@layer base {
  :root {
    ...
    --gradient: linear-gradient(135deg, #6b3e2e 0%, #c89666 100%);

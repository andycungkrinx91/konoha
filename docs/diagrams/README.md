...
| 03 MCP Tool and Skill Routing | `sannin` routing to tools and ninja agents (genin, jonin, anbu, tokubetsu-jonin, chunin embed `references/i-have-adhd.md`; sannin and kage do not; kage embeds `references/antislop*`); includes the `website_ai_detector` tool node (AI-fingerprint scan, 0-20 = Human-Built, CLI `konoha detect-ai`) | `docs/ARCHITECTURE.md` | `src/server.js`, `src/templates/agents.yaml`, `.agents/skills/*/references/i-have-adhd.md`, `src/ai_detector.js`, `src/mcp/ai_detector.js` |

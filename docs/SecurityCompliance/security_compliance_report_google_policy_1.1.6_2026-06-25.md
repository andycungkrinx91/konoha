...
### 22. Claude Code MCP Permissions
- **Action Verified**: Inspected [src/mcp_clients_manager.js](file:///home/andycungkrinx/experiment/portofolio/data/konoha/src/mcp_clients_manager.js) and [bin/cli.js](file:///home/andycungkrinx/experiment/portofolio/data/konoha/bin/cli.js). Verified that the global configuration setup for Claude Code now modifies `~/.claude/settings.json` to auto-allow tool execution by adding the `mcp__skills-db__*`, `mcp__konoha-files__*`, and `mcp__semble__*` tool name wildcards to the allowed list, which eliminates non-interactive execution aborts and user prompt blocks.
- **Impact**: Enables all Konoha MCP servers to run automatically and seamlessly within Claude Code.

### 23. Symmetric Configuration Ingestion and Uninstall Safety
- **Action Verified**: Inspected [src/cursor_manager.js](file:///home/andycungkrinx/experiment/portofolio/data/konoha/src/cursor_manager.js), [src/antigravity_manager.js](file:///home/andycungkrinx/experiment/portofolio/data/konoha/src/antigravity_manager.js), and [bin/cli.js](file:///home/andycungkrinx/experiment/portofolio/data/konoha/bin/cli.js). Verified that the installer and uninstaller processes merge JSON structures safely instead of replacing files. Confirmed that during uninstallation, only Konoha-specific command permissions are filtered and removed from settings files (`~/.cursor/cli-config.json`, `~/.gemini/antigravity-cli/settings.json`, and `~/.gemini/settings.json`), prompt hooks are unregistered from `~/.gemini/config/hooks.json`, and subagents are removed without touching existing user configurations.

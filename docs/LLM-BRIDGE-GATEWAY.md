...
Konoha automatically clones `https://github.com/andycungkrinx91/konoha-bridge` on fresh installation (`konoha init`), packages the extension into `konoha-bridge-1.6.0.vsix` via `@vscode/vsce package`, and installs the extension **only into Antigravity IDE** — it is never installed into any other IDE (VS Code, Cursor, etc.):

```bash
# Antigravity IDE CLI (the ONLY supported VSIX install target)
antigravity --install-extension assets/konoha-bridge-1.6.0.vsix
```

...
> * **Hard Guardrail Enforcement (v2.0.0-beta.7)**: The destructive-command, git-safety, secret-protection, and MCP read-bypass rules are enforced by blocking hooks, not just prompts — Claude Code (`konoha-bash-guard.js` on `^Bash$`), Command Code (`konoha-native-blocker-cc.js` on `read_file` + `shell_command`), Antigravity (`antigravity_tool_sanitize_hook.js` on `run_command`), and Pi (`konoha-blocker.ts` on reads + `bash`). Blocked shell commands can be retried with an `rtk` prefix (read-bypass only) or via the konoha/semble MCP tools.

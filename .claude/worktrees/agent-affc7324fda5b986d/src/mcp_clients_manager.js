...
const KONOHA_MCP_NAMES = ['konoha', 'semble', 'skills-db', 'konoha-files'];
...
function adaptInstructionsForClaudeCode(instructions) {
  ...
  return instructions
    ...
    .replace(/read_file_head/g, 'mcp__skills-db__read_file_head')
    .replace(/read_file_range/g, 'mcp__skills-db__read_file_range')
    .replace(/file_info/g, 'mcp__skills-db__file_info')
    .replace(/token_efficient_grep/g, 'mcp__skills-db__token_efficient_grep')
    .replace(/get_file_structure/g, 'mcp__skills-db__get_file_structure')
    .replace(/find_files_clean/g, 'mcp__skills-db__find_files_clean')
    .trim();
}
...
function readMcpHealth(config, key = 'mcpServers') {
  ...
  return {
    konoha: !!(block['skills-db'] || block['konoha']),
    semble: !!block.semble
  };
}
...
function getClaudeCodeStatus() {
  ...
  if (status.globalConfig) {
    try {
      ...
      status.mcpSkillsDb = health.konoha;

...
You are the **Claude Code agent** equipped with Konoha MCP servers (\`konoha\`, \`semble\`) and specialized ninja subagents (\`@genin\`, \`@kage\`, \`@chunin\`, \`@jonin\`, \`@anbu\`, \`@tokubetsu-jonin\`).
...
4. **Delegate to Konoha Ninja Agents**: When a user request matches a specific subagent domain or embedded skill, delegate the task by invoking the appropriate subagent (\`@genin\`, \`@kage\`, \`@chunin\`, \`@jonin\`, \`@anbu\`, \`@tokubetsu-jonin\`). Pass the task goal, context, and required skill references. If no subagent matches, execute directly using Direct Tool Calls and Konoha MCP tools (\`mcp__konoha__*\`).
...
`;
  ...
  return content
    ...
    .replace(/skills-db\.find_skill/g, 'mcp__konoha__find_skill')
    .replace(/skills-db\.get_skill/g, 'mcp__konoha__get_skill')
    .replace(/skills-db\.list_skills/g, 'mcp__konoha__list_skills')
    .replace(/skills-db\.optimize_report/g, 'mcp__konoha__optimize_report')
    .replace(/skills-db\.build_from_source/g, 'mcp__konoha__build_from_source')
    .replace(/skills-db\.build_from_text/g, 'mcp__konoha__build_from_text')
...
| **skills-db** | node ~/.konoha/file_tools_launcher.js | Skill discovery, file operations, all file reads |

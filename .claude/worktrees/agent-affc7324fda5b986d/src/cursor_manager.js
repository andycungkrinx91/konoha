...
function generateCursorSubagent(agent) {
  ...
  if (ruleContent) {
    return `---\ndescription: Konoha multi-agent orchestration — delegate to ninja subagents via Task tool, use konoha MCP for skills\nalwaysApply: true\n---\n\n` + ruleContent;
  }
    ...
    try {
      ...
      if (config.permissions && config.permissions.allow) {
        const grants = [
          ...
          'Mcp(semble, search)',
          'Mcp(semble, find_related)',
          'Mcp(skills-db, read_file_head)',
          'Mcp(skills-db, read_file_range)',
          'Mcp(skills-db, file_info)',
          'Mcp(skills-db, token_efficient_grep)',
          'Mcp(skills-db, get_file_structure)',
          'Mcp(skills-db, find_files_clean)',
          'Shell(konoha)',
          'Shell(node bin/cli.js)',
          'Shell(node */.konoha/cursor_bootstrap.js)'
        ];
...
function getCursorStatus() {
  ...
  if (status.mcpGlobal) {
    try {
      const config = JSON.parse(fs.readFileSync(CURSOR_MCP_GLOBAL, 'utf-8'));
      status.mcpSkillsDb = !!(config.mcpServers && (config.mcpServers['skills-db'] || config.mcpServers['konoha']));

...
function registerMcp(python, silent = false, allowAutoApprove = true) {
  ...
  if (konohaConfig) config.mcpServers['skills-db'] = konohaConfig;

  if (!silent) {
    success(`Registered 'semble' and 'skills-db' MCP servers.`);
  }
...
async function cmdDoctor() {
  ...
  // 5d. Token-efficient file tools (skills-db MCP)
  ...
  if (!fileExists(FILE_TOOLS_MCP_PATH)) {
    try {
      installFileTools(true);
      if (fileExists(FILE_TOOLS_MCP_PATH)) {
        record('File Tools MCP (skills-db)', 'REPAIRED', 'Installed skills-db MCP server and Python helpers');
        repairsDone++;
      } else {
        record('File Tools MCP (skills-db)', 'FAILED', 'file_tools_mcp.js missing after install');
        hasErrors = true;
      }
    } catch (e) {
      record('File Tools MCP (skills-db)', 'FAILED', e.message);
      hasErrors = true;
    }
  }
  ...
  if (cursorHealthy) {
    ...
    try {
      ...
      if (repaired.mcpSkillsDb && repaired.mcpSemble && repaired.subagentsGlobal >= 6) {
  ...
  if (mcpClientsManager.isClaudeCodeInstalled()) {
    ...
    if (claudeHealthy) {
      record('Claude Code (~/.claude.json)', 'HEALTHY', 'skills-db, semble, and permissions active & allowed');
    } else {
      try {
        const python = checkPython() || 'python3';
        const agents = agentManager.loadAgents();
        ...
        if (repaired.mcpSkillsDb && repaired.mcpSemble && repaired.permissionsAllowed) {
  ...
  if (mcpClientsManager.isOpenCodeInstalled()) {
    ...
    if (openCodeHealthy) {
      record('OpenCode (~/.config/opencode/)', 'HEALTHY', 'skills-db, semble active');
    } else {
      try {
        const python = checkPython() || 'python3';
        mcpClientsManager.ensureOpenCodeSetup({
        ...
        if (repaired.mcpSkillsDb && repaired.mcpSemble) {
...
Provider available during 'bridge create':
...
async function cmdBridgeModels() {
  ...
  if (!gatewayActive) {
    ...
    warn('Ensure the skills-db MCP server is running to start the gateway and enabled bridges.');
    return;
  }
...
async function cmdBridgeQuota() {
  const http = require('http');
  header('OpenAI OAuth Bridge Quota Status');

  const bridges = loadBridges().filter(b => b.provider === 'openai-oauth');
  if (bridges.length === 0) {
    log(`  No OpenAI OAuth bridges configured.\n`);
    log(`  Add one with: ${C.cyan}konoha bridge create${C.reset} (choose option 1 - OAuth Login)\n`);
    return;
  }

  // Try live query to gateway for runtime quota data
  let liveData = null;
  const gatewayActive = await checkPortActive(19999);
  if (gatewayActive) {
    liveData = await new Promise((resolve) => {
      const req = http.get('http://127.0.0.1:19999/v1/bridge-quota', (res) => {
        const chunks = [];
  ...
  if (liveData?.bridges) {
    for (const b of liveData.bridges) liveMap.set(b.name, b);
  }

  const rows = [];
  const oauthDir = path.join(os.homedir(), '.konoha');
  for (const b of bridges) {
    // Check token file for email
    let email = '-';
    try {
      const tokenFile = path.join(oauthDir, `oauth_${b.name}.json`);
      if (fileExists(tokenFile)) {
        const t = JSON.parse(fs.readFileSync(tokenFile, 'utf-8'));
        email = t.email || 'logged in';
      } else {
        email = '(no token)';
      }
    } catch {}

    const live = liveMap.get(b.name);
    const status    = live ? live.status : (gatewayActive ? 'Available' : 'Gateway offline');
    const resetTime = live?.unavailableUntil
      ? new Date(live.unavailableUntil).toLocaleTimeString()
      : '-';
    const remaining = live?.remainingSeconds > 0
      ? `${live.remainingSeconds}s`
      : '-';

    rows.push([b.name, String(b.port), email, status, resetTime, remaining]);
  }

  const headers = ['Bridge Name', 'Port', 'Account', 'Status', 'Resets At', 'Remaining'];
  const widths = computeTableWidths(headers, rows, { minWidths: [18, 6, 24, 12, 12, 10] });
  drawTable(headers, widths, ['left', 'left', 'left', 'left', 'left', 'right'], rows, [], RASENGAN_THEME, {
    columnFormatters: [
      (cell) => applyGradient(cell.trimEnd(), RASENGAN_THEME, 0.9) + cell.slice(cell.trimEnd().length),
      (cell) => cell,
      (cell) => `${C.dim}${cell}${C.reset}`,
      (cell) => cell.includes('Available') ? `${C.green}${cell}${C.reset}` : `${C.red}${cell}${C.reset}`,
      (cell) => cell,
      (cell) => cell !== '-' ? `${C.yellow}${cell}${C.reset}` : `${C.dim}${cell}${C.reset}`
    ]
  });

  if (!gatewayActive) {
    log(`\n  ${C.dim}Gateway not running \u2014 quota data shown from last known state.${C.reset}`);
    log(`  Start gateway: ${C.cyan}konoha bridge start${C.reset}\n`);
  } else {
    log(`\n  ${C.dim}Live data from gateway on port 19999.${C.reset}\n`);
  }
}
...
async function cmdBridgeCreate(name) {
  ...
  log(`    ${C.bold}1${C.reset}  OpenAI              ${C.dim}(https://api.openai.com/v1)${C.reset}`);
  log(`    ${C.bold}2${C.reset}  OpenAI Compatible   ${C.dim}(any OpenAI-compatible API)${C.reset}\n`);

  const provChoice = await askQuestion('  Provider [1/2, default 2]: ');
  const isOpenAI = provChoice.trim() === '1';

  let providerLabel, defaultTargetUrl;
  if (isOpenAI) {
    providerLabel = 'openai';
    defaultTargetUrl = 'https://api.openai.com/v1';
  } else {
  ...
  let targetUrl = '';
  while (!targetUrl) {
    const prompt = defaultTargetUrl
      ? `  Target URL [${defaultTargetUrl}]: `
      : '  Target URL (e.g. http://localhost:11434/v1): ';
    const input = await askQuestion(prompt);
    targetUrl = input || defaultTargetUrl;
    if (!targetUrl) warn('  Target URL is required.');
  }

  // API Key
  const apiKey = await askQuestion('  API Key (optional for local LLMs): ');
  ...
  if (apiKey) newBridge.apiKey = apiKey;

  saveBridgeSqlite('upsert', newBridge);
  log('');
  success(`Bridge "${bridgeName}" created!`);
  log(`  Provider : ${providerLabel === 'openai' ? 'OpenAI' : 'OpenAI Compatible'}`);

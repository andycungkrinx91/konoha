...
{#if isModalOpen}
  ...
  >
    ...
    <div
      class="w-full max-w-lg p-6 rounded-3xl border shadow-2xl space-y-4 animate-in zoom-in-95 duration-150"
      style="background-color: var(--color-surface); border-color: var(--color-border); color: var(--color-text); box-shadow: var(--shadow-3d);"
    ...
    >
      ...
      <div class="flex items-center justify-between pb-3 border-b" style="border-color: var(--color-border);">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            <h3 class="text-sm font-bold tracking-tight" style="color: var(--color-text);">Konoha Theme Palette Selector</h3>
            <p class="text-[11px]" style="color: var(--color-text-muted);">10 Light-Mode calibrated palettes with eye-friendly contrast</p>
          </div>
        </div>
        <button
          type="button"
          onclick={() => isModalOpen = false}
          class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border transition-colors hover:scale-105"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[60vh] overflow-y-auto pr-1">
        {#each THEMES as t}
          ...
          <button
            type="button"
            class="w-full flex items-center gap-3 p-3 rounded-2xl border text-left transition-all duration-150 hover:scale-[1.02] cursor-pointer"
            style="{isCurrent

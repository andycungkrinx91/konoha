...
### Added
- **Bridge Storage Migration (SQLite)**: `~/.konoha/bridges.json` flat-file storage is replaced by SQLite-backed persistence in `skills.db` (`bridges` table: `name`, `port`, `provider`, `enabled`, `target_url`, `api_key`, `quota_unavailable_until`). Legacy JSON entries are auto-migrated on first read. All bridge CLI commands (`konoha bridge create`, `delete`, `enable`, `disable`, `set-quota`, `clear-quota`) now route through `src/db_bridges.py`.
- **Quota-Aware Bridge Routing**: Bridges that return `RESOURCE_EXHAUSTED` or `429` errors are persisted in `quota_unavailable_until` state and excluded from `resolveBridgeAndModel()` until the timer expires. The gateway automatically rotates to the next available bridge (round-robin across `openai` provider slots).
...
### Changed
- **Proxy Gateway Port 11434 → 19999**: The outer Proxy Gateway now listens on port `19999` (was `11434`). Inner bridge default ports remain `11435` (antigravity) and `11436` (adacode). Update any client SDK base URLs accordingly.
...
- **`konoha init` zero-prompt auto-config**: Initialization now asks a single consent prompt ("Initialize Konoha and modify ~/.gemini configurations?") then auto-configures **all** detected MCP clients (Antigravity, Cursor, Claude Code, OpenCode) without further prompting. Previously each client required an individual Yes/No confirmation. Clients not detected on the system are skipped silently. `ensureAutoSetup()` (triggered by any `konoha` command) now also self-heals Cursor, Claude Code, and OpenCode configs in addition to Antigravity. A per-client summary is printed in the Installation Complete box.

### Added
- **New `adacode` OpenAI-Compatible Bridge (Port 11436)**: Ships a second default inner bridge alongside `antigravity`. The `adacode` bridge exposes a local OpenAI-compatible endpoint and forwards requests to a real upstream (e.g. `https://api.adacode.ai/v1`) using the API key stored in `~/.konoha/bridges.json`. Local clients connecting to the gateway on `19999` no longer need to send an API key — the bridge injects the key only on outbound upstream calls. Model prefix: `adacode-<model>` (e.g. `adacode-gpt-4o`).
...
- **Antigravity Auto-Spawn**: The `antigravity` bridge on port `11435` auto-spawns when the user's Antigravity IDE or `agy` CLI session is live, and releases the port when the session closes (combined with the lifecycle gate above).
...
- **Multi-Client Subagent Model Configuration**: Added the ability to view and configure models specifically for different MCP clients (Cursor, Claude Code, and OpenCode) via `konoha models list` and `konoha models embed`. Added `--cursor`, `--claude`, and `--opencode` flags to `konoha models embed`, along with a column layout in the models list view displaying Cursor, Claude Code, and OpenCode model configurations, while resolving model persistence reset issues.
...
- **OpenCode Status Column**: Updated the main `konoha status` subcommand to display the `Model (OpenCode)` column alongside Antigravity, Cursor, and Claude model assignments.
...
### Fixed
- **Claude Code MCP Namespacing Tool Calls**: Fixed a bug where Claude Code instructions (`~/.claude/CLAUDE.md` and `~/.claude/agents/*.md`) referenced MCP tools using dot-notation (e.g. `skills-db.find_skill`, `semble.search`). Since Claude Code names stdio MCP tools using double underscores (e.g. `mcp__skills-db__find_skill`, `mcp__semble__search`), the agent failed to resolve and invoke these tools. Added auto-rewriting mappings in `agent_manager.js` and `mcp_clients_manager.js` to replace standard dot-notation calls with the correct double-underscore prefixed format in all Claude Code configurations.
...
- **Config Ingestion and Uninstall Safety**: Verified and enhanced both installation/migration and uninstallation procedures across Cursor, Antigravity, Claude Code, and OpenCode. Config setup strictly injects MCP settings, whitelisted permissions, and subagents without modifying existing user preferences, and uninstallation surgically removes only Konoha-specific configurations, permissions, prompt hooks, and subagent files.
- **Claude Code MCP Permissions**: Resolved an issue where Konoha MCP servers (`skills-db`, `konoha-files`, `semble`) failed to execute inside Claude Code by implementing automatic user-level permission granting in `~/.claude/settings.json`.

...
- **SQLite-Driven Subagent & Model Configurations**: Migrated the core subagent definition and multi-client model mapping config (`agents.json`) to SQLite storage (`agents` table inside `skills.db`). Managed via `src/db_agents.py` CLI module, the database acts as the single source of truth for all client types (Antigravity, Cursor, Claude Code, and OpenCode), maintaining a synchronized mirror cache to ensure low-latency sandbox execution.
...
- **Claude Code & OpenCode Tool Whitelist Lockdown**: Removed `Read`, `Grep`, and `Glob` from the `allowed-tools` list in generated Claude Code and OpenCode configurations (`mcp_clients_manager.js`), preventing agents from bypassing konoha MCP.
...
- **OpenCode Subagents Markdown Configurations**: Added dynamic subagent deployment for OpenCode (similar to Claude Code). When the OpenCode CLI is detected, the 6 ninja subagents (`genin.md`, `kage.md`, `chunin.md`, `jonin.md`, `anbu.md`, and `tokubetsu-jonin.md`) are now generated and deployed dynamically under `~/.config/opencode/agents/` using YAML frontmatter permissions and adapted prompt routing.
